...
class AutoForensic():
	...
	def pyzip(self):
		zip_path = input("input zip path (Autoexec.zip): ")
		passlib = open("./pw.txt","r").readlines()
		passlib = [s.strip() for s in passlib]
		print(passlib)
		with pyzipper.AESZipFile(zip_path, 'r') as zip_file:
			if zip_file.pwd is not None:
				zip_file.extractall('path')
			else:
				for k in passlib:
					try:
						zip_file.pwd = bytes(k, 'utf-8')
						zip_file.extractall(path='.', pwd=bytes(k, 'utf-8'))
						print("비밀번호를 찾았습니다 : "+k)
						print("압축해제 성공!")
						break
